...
class ApplicationState(object):
        ...
        try:
            while self.is_alive:
                ...
                for ws_client in self.get_ws_clients().values():
                    self.logger.debug("Sending message to websocket, websocket_id=%s", ws_client.websocket_id)
                    future = asyncio.run_coroutine_threadsafe(
                        ws_client.websocket.send_bytes(message_bytes), self._loop)
    ...
    async def dispatch_tip_filter_notifications_async(self, matches: list[PushdataRowParsed],
        ...
        rows = self.mysql_db_tip_filter_queries\
            .read_indexer_filtering_registrations_for_notifications(list(matches_by_hash))
        ...
        metadata_by_account_id = { row.account_id: row for row
            in self.mysql_db_tip_filter_queries.read_account_metadata(list(matches_by_account_id)) }
