...
from typing import Any, cast, NamedTuple, Optional, Sequence, ParamSpec, TypeVar, Type, Collection
...
def read_rows_by_id(mysqldb: MySQLDatabase, return_type: Type[T1], sql: str, params: Sequence[Any],
...
def read_rows_by_ids(mysqldb: MySQLDatabase, return_type: Type[T1], sql: str, sql_condition: str,
...
class MySQLTipFilterQueries:
    ...
    def create_account_write(self, external_account_id: int) -> int:
        ...
        try:
            cursor.execute("SELECT account_id FROM accounts WHERE external_account_id=%s",
                (external_account_id,))
            row = cursor.fetchone()
            if row is not None:
    ...
    def read_account_metadata(self, account_ids: list[int]) \
    ...
    def read_account_id_for_external_account_id(self, external_account_id: int) \
            -> int:
        cursor: MySQLdb.cursors.Cursor = self.mysql_conn.cursor()
        cursor = cursor.execute("SELECT account_id FROM accounts WHERE external_account_id=%s",
            (external_account_id,))
        row = cursor.fetchone()
        assert row is not None
        return cast(int, row[0])
    ...
    def get_spent_outpoints(self, entries: list[OutpointType], lmdb: LMDB_Database) \
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                output_tx_location = TxLocation(out_block_hash, out_block_num,
                    out_tx_position)
                out_tx_hash = _get_full_tx_hash(output_tx_location, lmdb)
                assert out_tx_hash is not None
                output_spend_row = OutputSpendRow(out_tx_hash, out_idx, in_tx_hash, in_idx,
                    in_block_hash)
    ...
    def create_tip_filter_registrations_write(self, external_account_id: int,
            date_created: int, registration_entries: list[TipFilterRegistrationEntry]) -> bool:
        ...
        for entry in registration_entries:
            insert_rows.append((account_id, entry.pushdata_hash,
                int(IndexerPushdataRegistrationFlag.FINALISED), date_created, date_created +
    ...
    def read_tip_filter_registrations(self,
        ...
        sql = "SELECT pushdata_hash, date_expires FROM tip_filter_registrations " \
            "WHERE flags&%s=%s"
    ...
    def read_indexer_filtering_registrations_for_notifications(self,
        ...
        """
        sql = "SELECT account_id, pushdata_hash FROM tip_filter_registrations " \
        ...
        return read_rows_by_id(self.mysql_db, FilterNotificationRow, sql, sql_values,
            pushdata_hashes)
    ...
    def update_tip_filter_registrations_flags_write(self,
        ...
        for pushdata_value in pushdata_hashes:
            update_rows.append((int(final_update_mask), int(update_flags), account_id, pushdata_value,
                int(final_filter_mask), int(filter_flags)))
        cursor: MySQLdb.cursors.Cursor = self.mysql_conn.cursor()
        try:
    ...
    def expire_tip_filter_registrationss(self, date_expires: int) -> list[bytes]:
        ...
        """
        sql_deletion = "DELETE FROM tip_filter_registrations " \
            "WHERE flags&%s=%s AND date_expires<%s"
        sql_select = "SELECT pushdata_hash FROM tip tip_filter_registrations " \
                     "WHERE flags&%s=%s AND date_expires<%s"
    ...
    def read_pending_outbound_datas(self, flags: OutboundDataFlag,
    ...
    def update_outbound_data_last_tried_write(self, entries: list[tuple[OutboundDataFlag, int, int]]) -> None:
        sql = "UPDATE outbound_data SET outbound_data_flags=%s, date_last_tried=%s " \
            "WHERE outbound_data_id=%s"
        cursor: MySQLdb.cursors.Cursor = self.mysql_conn.cursor()
        try:
            entries_with_flag_as_int: list[tuple[int, int, int]] = [(int(outbound_data_flags),
                date_last_tried, outbound_data_id) for outbound_data_flags, date_last_tried,
                outbound_data_id in entries]
