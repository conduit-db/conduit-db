    ...
    def wait_for_batch_completion(self, blocks_batch_set: set[bytes]) -> None:
        while True:
            ...
            if block_hash in blocks_batch_set:
                ...
                self.logger.error(
                    f"also wrote unexpected block: {hash_to_hex_str(header.hash)}, "
                    f"height={header.height} to disc"
                )

            # all blocks in batch processed
            if len(blocks_batch_set) == 0:
                self.loop.call_soon_threadsafe(self.controller.done_blocks_mtree_event.set)
                break
