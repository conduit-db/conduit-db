...
class TxParser(multiprocessing.Process):
    ...
    def run(self) -> None:
        ...
        self.kill_worker_socket = connect_non_async_zmq_socket(
            self.zmq_context,
            "tcp://127.0.0.1:63241",
            zmq.SocketType.SUB,
    ...
    def setup_tip_filtering(self) -> None:
        ...
        self.logger.debug(
            "Populated the common cuckoo filter with %d entries",
        ...
        self.socket_utxo_spend_registrations = connect_non_async_zmq_socket(
            self.zmq_context,
            f"tcp://{ZMQ_CONNECT_HOST}:60000",
            zmq.SocketType.SUB,
            options=[(zmq.SocketOption.SUBSCRIBE, b"utxo_registration")],
        )
        self.socket_utxo_spend_notifications = connect_non_async_zmq_socket(
            self.zmq_context,
            f"tcp://{ZMQ_CONNECT_HOST}:60001",
            zmq.SocketType.PUSH,
        )
        self.socket_pushdata_registrations = connect_non_async_zmq_socket(
            self.zmq_context,
            f"tcp://{ZMQ_CONNECT_HOST}:60002",
            zmq.SocketType.SUB,
            options=[(zmq.SocketOption.SUBSCRIBE, b"pushdata_registration")],
        )
        self.socket_pushdata_notifications = connect_non_async_zmq_socket(
            self.zmq_context,
            f"tcp://{ZMQ_CONNECT_HOST}:60003",
            zmq.SocketType.PUSH,
        )

        request_id = os.urandom(16).hex()
        state_update_to_server = OutpointStateUpdate(
            request_id, OutpointMessageType.READY, None, None, self.worker_id
        )
    ...
    def send_utxo_spend_notifications(
        self, in_rows_parsed: list[InputRowParsed], blk_hash: bytes | None
    ) -> None:
        if blk_hash is None:
            blk_hash = bytes(32)
        ...
        for input_row in in_rows_parsed:
            ...
            if spent_output in self.unspent_output_registrations:
                request_id = os.urandom(16).hex()
                notification = OutpointStateUpdate(
                    request_id,
                    OutpointMessageType.SPEND,
                    None,
                    output_spend_struct.pack(*input_row, blk_hash),
                    self.worker_id,
                )
    ...
    def send_pushdata_match_notifications(
        self,
        ...
        if len(filter_matches):
            ...
            request_id = os.urandom(16).hex()
            notification_pushdata = PushdataFilterStateUpdate(
                request_id,
                PushdataFilterMessageType.NOTIFICATION,
                [],
                filter_matches,
                blk_hash,
            )
