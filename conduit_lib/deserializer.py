...
class Deserializer:
    ...
    def version(self, f: BytesIO) -> Version:
        ...
        return Version(version=version, services=services, timestamp=timestamp, addr_recv=addr_recv,
            addr_from=addr_from, nonce=nonce, user_agent=user_agent, start_height=start_height,
            relay=relay)
    ...
    def protoconf(self, f: BytesIO) -> Protoconf:
        ...
        return Protoconf(number_of_fields=number_of_fields,
    ...
    def addr(self, f: BytesIO) -> list[NodeAddrListItem]:
        ...
        for i in range(count):
            ...
            addresses.append(NodeAddrListItem(timestamp=timestamp,
