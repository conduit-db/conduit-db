...
class RedisCache:
    ...
    def __init__(self) -> None:
        self.r = redis.Redis(host=os.getenv('SCYLLA_HOST', '127.0.0.1'),
    ...
    def get_in_namespace(self, namespace: bytes, key: bytes) -> Any:
    ...
    def scan_in_namespace(self, namespace: bytes) -> list[bytes]:
        ...
        all_keys = []
        while cursor != 0:
            cursor, keys = self.r.scan(cursor=cursor if cursor else 0, match=namespace + b"*",
                count=self.BATCH_SIZE)
            all_keys.extend(keys)
        return all_keys

    def bulk_load_in_namespace(self, namespace: bytes, pairs: Sequence[tuple[Any, Any]]):
    ...
    def bulk_delete_in_namespace(self, prefix: bytes) -> None:
        with Timer(name='redis-pipeline-delete') as timer:
            pipe = self.r.pipeline()
            cursor = None
            timer.count = 0
            while cursor != 0:
                cursor, keys = self.r.scan(cursor=cursor if cursor else 0, match=prefix+b"*",
                    count=self.BATCH_SIZE)
