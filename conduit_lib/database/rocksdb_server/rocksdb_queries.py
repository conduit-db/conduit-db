...
from conduit_lib.database.types import ConfirmedTransactionRow, MinedTxHashes
from ...constants import PROFILING
from ...types import ChainHashes

if typing.TYPE_CHECKING:
    from .rocksdb_database import RocksDbDatabase


class RocksDbQueries:

    def __init__(self, db: Rdict, rocksdb_tables: RocksDbTables, bulk_loads:
            RocksDbBulkLoads, rocksdb_db: 'RocksDbDatabase') -> None:
    ...
    def rocksdb_load_temp_mined_tx_hashes(self, mined_tx_hashes: list[MinedTxHashes]) -> None:
        ...
        self.rocksdb_tables.rocksdb_create_temp_mined_tx_hashes_table()
    ...
    def rocksdb_load_temp_inbound_tx_hashes(self, inbound_tx_hashes: list[tuple[str]],
        ...
        self.rocksdb_tables.rocksdb_create_temp_inbound_tx_hashes_table(inbound_tx_table_name)
    ...
    def rocksdb_get_unprocessed_txs(self, is_reorg: bool, new_tx_hashes: list[tuple[str]],
        ...
        """
        self.rocksdb_load_temp_inbound_tx_hashes(new_tx_hashes, inbound_tx_table_name)
        try:
            self.rocksdb_conn.query(
                f"""
            ...
            self.rocksdb_db.commit_transaction()

        if not is_reorg:
            self.rocksdb_tables.rocksdb_drop_temp_inbound_tx_hashes(inbound_tx_table_name)
            return final_result
        else:
            try:
                self.rocksdb_conn.query(f"""SELECT * FROM temp_orphaned_txs;""")
                # Todo - Where tx_hash = inbound_tx_table_name.inbound_tx_hashes
                result = self.rocksdb_conn.store_result()
    ...
    def rocksdb_load_temp_mempool_removals(self, removals_from_mempool: set[bytes]) -> None:
        ...
        self.rocksdb_tables.rocksdb_create_temp_mempool_removals_table()
    ...
    def rocksdb_load_temp_mempool_additions(self, additions_to_mempool: set[bytes]) -> None:
        self.rocksdb_tables.rocksdb_create_temp_mempool_additions_table()
    ...
    def rocksdb_load_temp_orphaned_tx_hashes(self, orphaned_tx_hashes: set[bytes]) -> None:
        self.rocksdb_tables.rocksdb_create_temp_orphaned_txs_table()
    ...
    def rocksdb_remove_from_mempool(self) -> None:
        ...
        self.rocksdb_conn.query(query)
        self.rocksdb_conn.commit()
        self.rocksdb_tables.rocksdb_drop_temp_mempool_removals()
    ...
    def rocksdb_add_to_mempool(self) -> None:
        ...
        self.rocksdb_conn.query(query)
        self.rocksdb_conn.commit()
        self.rocksdb_tables.rocksdb_drop_temp_mempool_additions()
    ...
    def rocksdb_update_checkpoint_tip(self, checkpoint_tip: bitcoinx.Header) -> None:
        try:
            ...
            self.rocksdb_db.start_transaction()
            query = f"""
    ...
    # def rocksdb_delete_pushdata_rows(self, pushdata_rows: list[PushdataRow]) -> None:
    #     t0 = time.time()
    #     cursor: MySQLdb.cursors.Cursor = self.rocksdb_conn.cursor()
    ...
    #     )
    #
    # def rocksdb_delete_output_rows(self, output_rows: list[OutputRow]) -> None:
    #     t0 = time.time()
    #     cursor: MySQLdb.cursors.Cursor = self.rocksdb_conn.cursor()
    #     cursor.execute("START TRANSACTION;")
    #     try:
    ...
    #     )
    #
    # def rocksdb_delete_input_rows(self, input_rows: list[InputRow]) -> None:
    #     t0 = time.time()
    #     cursor: MySQLdb.cursors.Cursor = self.rocksdb_conn.cursor()
    #     cursor.execute("START TRANSACTION;")
    #     try:
    ...
    #     )
    #
    # def rocksdb_delete_header_row(self, block_hash: bytes) -> None:
    #     t0 = time.time()
    #     self.rocksdb_db.start_transaction()
    #     try:
    #         query = f"""
    ...
    def get_mempool_size(self) -> int:
        ...
        self.rocksdb_db.rocksdb_conn.query(sql)
        result = self.rocksdb_db.rocksdb_conn.store_result()
