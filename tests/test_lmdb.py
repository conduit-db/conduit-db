...
class TestLMDBDatabase:
    ...
    def test_block_storage(self) -> None:
        ...
        _block_num, _len_slice, raw_block_slice = struct.unpack_from(
            f"<IQ{len_slice}s", batched_block_slices, 0
        )
    ...
    def test_merkle_tree_storage(self) -> None:
        ...
        middle_level = [
            double_sha256(base_level[0] + base_level[1]),
            double_sha256(base_level[2] + base_level[2]),
        ]
