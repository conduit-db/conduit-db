...
class SessionManager:
    ...
    async def connect(self):
        ...
        protocol_factory = lambda: BufferedSession(self.config, peer, self.host, self.port)
        self.transport, self.session = await loop.create_connection(protocol_factory, peer.host,
            peer.port)
    ...
    async def run(self):
        try:
            await self.connect()
            init_handshake = asyncio.create_task(self.session.send_version(
                recv_host=self.session.peer.host,
                recv_port=self.session.peer.port, send_host=self.session.host,
                send_port=self.session.port))
            wait_untiL_conn_lost = asyncio.create_task(self.session.con_lost_event.wait())
