...
class BufferedSession(BufferedProtocol):
    ...
    def __init__(self, config: NetworkConfig, peer: Peer, host, port):
        ...
        self.con_lost_event = asyncio.Event()  # waited on by session manager at higher LOA
    ...
    def _new_buffer(self, size):
        self.buffer = bytearray(size)  # not sure this is necessary. can maybe re-use if pos correct
        self.buffer_view = memoryview(self.buffer)  # can swap to shared_memory for multi-cpu reads
        self.pos = 0
    ...
    def buffer_updated(self, nbytes):
        ...
        # get payload
            ...
            if is_block_msg(current_command):  # start work early on memory view of the block
                self.allocate_work(self.buffer_view, self.pos, self._payload_size, current_command)
    ...
    async def send_version(self, recv_host=None, recv_port=None, send_host=None, send_port=None):
        message = self.serializer.version(recv_host=recv_host, recv_port=recv_port,
    ...
    def allocate_work(self, buffer_view: memoryview, pos: int, payload_size: int,
            current_command: str):
    ...
    def parse_block_partition(self, buffer_view: memoryview, pos: int, payload_size: int,
            current_command: str):
        def parse_block():
