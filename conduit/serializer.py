...
from bitcoinx import pack_le_uint32, pack_varint, pack_le_uint64, pack_le_int64, pack_varbytes, \
    hex_str_to_hash, int_to_be_bytes, pack_be_uint16, pack_byte

from commands import VERSION_BIN, VERACK_BIN, GETADDR_BIN, FILTERCLEAR_BIN, GETHEADERS_BIN, \
    FILTERLOAD, INV_BIN, TX_BIN, GETDATA_BIN
from networks import NetworkConfig
from utils import payload_to_checksum, ipv4_to_mapped_ipv6, flip_hex_byte_order, \
    calc_bloom_filter_size, hex_to_bytes
...
class Serializer:
    ...
    def version(self, recv_host: str, send_host: str, recv_port: int=8333, send_port:
            int=8333, version=70015, relay=1):
        ...
        payload = version + services + timestamp + addr_recv + addr_sndr + nonce + \
            user_agent + height + relay
    ...
    def getheaders(self, hash_count: int, block_locator_hashes: str, hash_stop=ZERO_HASH):
