...
def get_peerinfo():
    payload = json.dumps({"jsonrpc": "2.0", "method": f"getpeerinfo", "params": [], "id": 0})
    result = requests.post(f"http://rpcuser:rpcpassword@127.0.0.1:8332", data=payload)
    return result.json()["result"]


def setban(ip):
    payload = json.dumps({"jsonrpc": "2.0", "method": f"setban", "params": [ip, "add"], "id": 0})
    result = requests.post(f"http://rpcuser:rpcpassword@127.0.0.1:8332", data=payload)
    return result.json()["result"]


def listbanned():
    payload = json.dumps({"jsonrpc": "2.0", "method": f"listbanned", "params": [], "id": 0})
    result = requests.post(f"http://rpcuser:rpcpassword@127.0.0.1:8332", data=payload)
    return result.json()["result"]
